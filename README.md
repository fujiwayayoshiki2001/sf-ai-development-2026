...
このプロジェクトには **sf-explore** という subagent が定義されています。
コードベースを横断的に調べる時に、独立したコンテキストで動作します。

以下のような場面で、自動または明示的に起動されます:

- 「このシステムの全体像を教えて」
- 「スコア計算の流れを追跡して」
- 「新機能を実装する前に既存の類似実装を調べて」

subagent は多数のファイルを読んで **要点だけを要約** して返すため、
メインの会話コンテキストを消費せずに深い調査ができます。

subagent の定義は `.claude/agents/sf-explore.md` で確認できます。
`/agents` コマンドで現在利用可能な subagent を一覧表示できます。
